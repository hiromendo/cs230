...
class WaveNetClassifier():
  def __init__(self, input_shape, output_shape, kernel_size = 2, dilation_depth = 9, n_filters = 40, task = 'classification', regression_range = None, load=False, load_dir='./'):
    """
    Parameters:
      input_shape: (tuple) tuple of input shape (for a single sample). (e.g. If input is 6s raw waveform with sampling rate = 16kHz, (96000,) is the input_shape)
  ...
  def residual_block(self, x, i):
    ...
    #element-wise multiply tanh and sigm layer outputs (apply the gate to the filter)
    z = Multiply(name='gated_activation_%d' % (i))([tanh_out, sigm_out])
    #set up the residual (skip) connection
    skip = Conv1D(self.n_filters, 1, name='skip_%d'%(i))(z)
    res = Add(name='residual_block_%d' % (i))([skip, x])
    #return the residual block output and the skip connection
    return res, skip
  ...
  def construct_model(self):    
    ...
    if self.expand_dims == True:
      ...
      x_reshaped = x
    skip_connections = []
    #apply a causal convolution (initial masking layer)
    out = Conv1D(self.n_filters, 2, dilation_rate=1, padding='causal', name='dilated_conv_1')(x_reshaped)
    #for the range of the dilation depth, apply a residual block
    for i in range(1, self.dilation_depth + 1):
      out, skip = self.residual_block(out,i)
      #store the residual connections
      skip_connections.append(skip)
    #add in the residual connections to the model
    out = Add(name='skip_connections')(skip_connections)
    #apply a relu activation to the skip connections
    out = Activation('relu')(out)
    #apply a 1x1 convolution to reduce the filter dimensionality
    out = Conv1D(self.n_filters, 80, strides = 1, padding='same', name='conv_5ms', activation = 'relu')(out)
    #apply a pooling layer to reduce the feature space dimensionality
    ...
    if self.time_distributed:
      ...
      #apply time distributed with self.activation = softmax to get prediction
